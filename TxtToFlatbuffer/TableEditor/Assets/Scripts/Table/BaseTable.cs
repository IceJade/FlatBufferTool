...
namespace LF
{
	...
	public class BaseTable
    {
        ...
        protected virtual void LoadTable()
        {
            ...
            {
                Debug.LogErrorFormat("The file {0} is not load, please check tables assetbundle", this.GetTableFileName());
            }
#endif
#else
            ...
            using (UnityWebRequest request = UnityWebRequest.Get(filepath))
            {
                ...
                while (!request.downloadHandler.isDone)
                {
                    if (request.isNetworkError || request.isHttpError)
                    {
                        ...
                        Debug.LogErrorFormat("Load {0} fail, error mssage : {1}", request.url, request.error);
                    ...
                    {
                        this.load_state = E_LoadState.Fail;
                        Debug.LogErrorFormat("Load table {0} time out.", filepath);
            ...
            {
                this.load_state = E_LoadState.Fail;
                Debug.LogErrorFormat("Read table {0} exception, error msg : {1}", filepath, e.Message);
            }
#endif
            ...
            {
                this.load_state = E_LoadState.Fail;
                Debug.LogErrorFormat("Load table {0} fail.", filepath);
            }
        ...
        protected virtual void LoadIndex()
        {
            ...
            {
                Debug.LogErrorFormat("The file {0} is not load, please check tables assetbundle", this.GetIndexFileName());
            }
#endif
#else
        ...
        protected virtual string GetTableFile()
        {
            ...
            if (IsUseAbTest () || IsUseJapanseMovie() || IsUseCustomMovie ())
            {
                ...
                {
                    // 如果读写区存在表文件, 那么使用此文件
                    string abFullPathFile = Path.Combine(this.GetDataReadWritePath(), "Tables", abFileName);
            ...
            {
                // 如果读写区存在表文件, 那么使用此文件
                string fullPathFile = Path.Combine(this.GetDataReadWritePath(), "Tables", dataFileName);
        ...
        protected virtual string GetIndexFile()
        {
            ...
            if (IsUseAbTest () || IsUseJapanseMovie() || IsUseCustomMovie ())
            {
                ...
                {
                    // 如果读写区存在表文件, 那么使用此文件
                    string abFullPathFile = Path.Combine(this.GetDataReadWritePath(), "Tables/ids", abFileName);
                    if (File.Exists(abFullPathFile))
                    {
#if !UNITY_EDITOR && !UNITY_IOS
                        return "file://" + abFullPathFile;  
                    ...
                    {
                        Debug.LogError($"临时目录没有找到配表的索引文件:{abFullPathFile}");
                    }
                }
            ...
            {
                // 如果读写区存在表文件, 那么使用此文件
                string fullPathFile = Path.Combine(this.GetDataReadWritePath(), "Tables/ids", indexFileName + ".bytes");
                if (File.Exists(fullPathFile))
                {
#if !UNITY_EDITOR && !UNITY_IOS
                    return "file://" + fullPathFile;
