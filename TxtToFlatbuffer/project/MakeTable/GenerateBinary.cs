...
namespace MakeTable
{
    ...
    class GenerateBinary
    {
        ...
        /// <returns></returns>
        public bool MakeBinary(string txtPath, string genPath)
        {
            Log.Print("---------------------------------------------------------------------------------------");
            ...
            {
                // 生成二进制文件
                this.CreateDataFile(txtPath, genPath);

                // 创建ID和行数的索引文件;
                this.CreateIndexIdDicFile(txtPath, genPath);
            }
            else
            {
                // 遍历文件夹;
        ...
        /// <param name="genPath"></param>
        private bool CreateDataFile(string txtFile, string genPath)
        {
            ...
            try
            {
                ...
                // 初始化内存长度
                FlatBufferBuilder fbBuilder = new FlatBufferBuilder((lineCount - 2) * 512);
                // 临时数组,包含注释行
                Offset<int>[] tmpOffsets = new Offset<int>[lineCount - 2];
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            if(lineIndex >= 3)
                            {
                                ...
                                for (int j = 0; j < columnCount; j++)
                                {
                                    ...
                                    if (this.TableColumnInfoList[j].columnIndex > datas.Length)
                                    {
                                        Log.Error("column index is error, column:{0}, row:{1}, file:{2}", j + 1, lineIndex + 1, txtFile);
                                        ErrorLog.Error("column index is error, column:{0}, row:{1}, file:{2}", j + 1, lineIndex + 1, txtFile);
                                    ...
                                    if(this.TableColumnInfoList[j].columnIndex < datas.Length)
                                    {
                                        columnValue = datas[this.TableColumnInfoList[j].columnIndex];
                                    }
                                    else
                                    {
                                        Log.Error("数据错误! data:{0} file:{1}", line, fileName);
                                        ErrorLog.Error("数据错误! data:{0} file:{1}", line, fileName);
                                    ...
                                    if (index >= ushort.MaxValue)
                                    {
                                        ...
                                        ErrorLog.Error("config error => Row counts of {0} is over max limit, max:{1}", txtFile, ushort.MaxValue);
                                    }
                                    ...
                                    if (columnName.ToLower() == Const.g_Id && index <= ushort.MaxValue)
                                    {
                                        ...
                                        if (null != find)
                                        {
                                            Log.Error("The id is repeat, table:{0}, id:{1}", txtFile, columnValue);
                                            ErrorLog.Error("config error => The id {0} is repeat, file:{1}", columnValue, txtFile);
                                        }
                                        ...
                                        if (!string.IsNullOrEmpty(columnValue.Trim()))
                                        {
                                            ...
                                            if (long.TryParse(columnValue, out longValue))
                                            {
                                                ...
                                                {
                                                    Log.Error("The value of id must be less than int.MaxValue[{0}], table:{1}, id:{2}", int.MaxValue, txtFile, columnValue);
                                                    Log.Error("config error => The value of id must be less than int.MaxValue[{0}], table:{1}, id:{2}", int.MaxValue, txtFile, columnValue);
                                                }
                                            }
                                            //else
                                            //{
                                        ...
                                        {
                                            Log.Error("The value of id must be not empty or space, table:{0}, id:{1}", txtFile, columnValue);
                                            Log.Error("config error => The value of id must be not empty or space, table:{0}, id:{1}", txtFile, columnValue);
                                ...
                                if (columnValueList.Count > 0)
                                {
                                    ...
                                    // 结束行
                                    if(rowIndex < tmpOffsets.Length)
                                        tmpOffsets[rowIndex] = new Offset<int>(fbBuilder.EndTable());
                                    else
                                    {
                                        Log.Error("out of array size, file:{0}", txtFile);
                                        ErrorLog.Error("out of array size, file:{0}", txtFile);
                                    }
            ...
            {
                Log.Error("Parse file error, error message:{0}, file:{1}", e.Message, txtFile);
                ErrorLog.Error("Parse file error, error message:{0}, file:{1}", e.Message, txtFile);
            }
