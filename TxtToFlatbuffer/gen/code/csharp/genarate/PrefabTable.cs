...
namespace Chanto.Table
{
	...
	public sealed class PrefabTable : StringIdConfig
    {
        private Table_prefab table = default(Table_prefab);

        private Dictionary<string, PrefabDataRow> data_row = new Dictionary<string, PrefabDataRow>(128);
        
        public static string g_TableFileName = "prefab";
        ...
        {
            Table_prefab table = this.GetTable();
        ...
        /// <returns>行数据</returns>
        public PrefabDataRow GetDataRow(string id)
        {
            ...
            {
                DRprefab? data = table.Data(row_index[hashcode]);
                if (data.HasValue && data.Value.Id == id)
                {
                    PrefabDataRow datarow = new PrefabDataRow(data.Value, row_index[hashcode]);
            ...
            {
                DRprefab? data = table.Data(i);
                if (data.HasValue)
                {
                    PrefabDataRow datarow = new PrefabDataRow(data.Value, i);
                    if(!data_row.ContainsKey(datarow.Id))
                        data_row.Add(datarow.Id, datarow);
        ...
        /// <returns></returns>
        public PrefabDataRow GetDataRowByIndex(int index)
        {
            Table_prefab table = this.GetTable();
            if (null == table.ByteBuffer)
                return null;
            ...
            DRprefab? data = table.Data(index);
            if (data.HasValue)
            {
                if (!data_row.ContainsKey(data.Value.Id))
                {
                    PrefabDataRow datarow = new PrefabDataRow(data.Value, index);
                    data_row.Add(datarow.Id, datarow);
                }
        ...
        public Dictionary<string, PrefabDataRow> GetAllData()
        {
            this.LoadTable();
        ...
        {
            table = Table_prefab.GetRootAsTable_prefab(byteBuffer);
    ...
    public sealed class PrefabDataRow : BaseDataRow
    {
        private DRprefab _datarow;

        public PrefabDataRow(DRprefab datarow, int index) : base(index)
        {
        ...
        protected override LuaValue GetLuaValue(string rowId)
        {
            ...
            switch (rowId)
            {
                ...
                case "DefaultPrefab": { luaValue.SetValue(this.DefaultPrefab); break; }
                case "Prefabs": { luaValue.SetValue(this.PrefabsArray); break; }
        ...
        private string _DefaultPrefab = null;
        public string DefaultPrefab { get { if (null == _DefaultPrefab) _DefaultPrefab = _datarow.DefaultPrefab; return _DefaultPrefab; } }

        private string[] _PrefabsArray = null;
        public string[] PrefabsArray { get { if(null == _PrefabsArray) { if (_datarow.PrefabsLength > 0) { _PrefabsArray = new string[_datarow.PrefabsLength]; for (int i = 0; i < _datarow.PrefabsLength; i++) { _PrefabsArray[i] = _datarow.Prefabs(i); } } } return _PrefabsArray; } }
        public int PrefabsArrayLength { get { if (null == this.PrefabsArray) return 0; return this.PrefabsArray.Length; } }
