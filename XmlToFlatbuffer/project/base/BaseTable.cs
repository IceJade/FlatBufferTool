...
using GameFramework;
using Shelter.Scripts.Game.AppFlyerSwitchMovie;
...
namespace LF
{
	...
	public class BaseTable
    {
        ...
        private string JPTablePrefix = "JP_";
        private bool isReadStreamingAssetsPathTable = true;
        ...
        public virtual int GetDataCount() { return 0; }
        ...
        public virtual BaseDataRow GetTableRowByIndex(int index) { return null; }
        ...
        protected virtual string GetTableFile()
        {
...
#if USE_TABLE_ASSET_BUNDLE_MODE
            if (GameEntry.Controller.ABTest.IsUseABTestConfig(this.GetTableFileName()))
            {
...
#else
            if (IsUseAbTest () || IsUseJapanseMovie() || IsUseCustomMovie ())
            {
                string abFileName = string.Empty;
                if (IsUseAbTest())
                    abFileName = ABTablePrefix + dataFileName;
                else if(IsUseJapanseMovie ())
                    abFileName = JPTablePrefix + dataFileName;
                else if(IsUseCustomMovie ())
                    abFileName = AppFlyerSwitchMovieController.Logic.GetMovieTablePrefix () + dataFileName;


                if(!GameEntry.DataTable.UseLocalDataTable)
                {
                    // 如果读写区存在表文件, 那么使用此文件
                    string abFullPathFile = Path.Combine(CommonUtils.GetInternalDir(), "Tables", abFileName);
                    if (File.Exists(abFullPathFile))
                    {
                        this.isReadStreamingAssetsPathTable = false;
...
#if UNITY_EDITOR
                return Path.Combine(Application.dataPath, "DataTable", "Tables", abFileName);
#else
                return Path.Combine(Application.streamingAssetsPath, "Tables", abFileName);
#endif
            }
            ...
            {
                // 如果读写区存在表文件, 那么使用此文件
                string fullPathFile = Path.Combine(CommonUtils.GetInternalDir(), "Tables", dataFileName);
            ...
            this.isReadStreamingAssetsPathTable = true;

#if UNITY_EDITOR
            return Path.Combine(Application.dataPath, "DataTable/Tables", dataFileName);
#else
        ...
        protected virtual string GetIndexFile()
        {
            ...
            if (IsUseAbTest () || IsUseJapanseMovie() || IsUseCustomMovie ())
            {
                string abFileName = string.Empty;
                if (IsUseAbTest())
                    abFileName = ABTablePrefix + indexFileName + ".bytes";
                else if(IsUseJapanseMovie ())
                    abFileName = JPTablePrefix + indexFileName + ".bytes";
                else if (IsUseCustomMovie ())
                    abFileName = AppFlyerSwitchMovieController.Logic.GetMovieTablePrefix () + indexFileName + ".bytes";

                if (!this.isReadStreamingAssetsPathTable)
                {
                    // 如果读写区存在表文件, 那么使用此文件
                    string abFullPathFile = Path.Combine(CommonUtils.GetInternalDir(), "Tables/ids", abFileName);
                    if (File.Exists(abFullPathFile))
                    {
#if !UNITY_EDITOR && !UNITY_IOS
                        return "file://" + abFullPathFile;  
                    ...
                    {
                        Log.ReleaseError($"临时目录没有找到配表的索引文件:{abFullPathFile}");
                    }
                }

#if UNITY_EDITOR
                return Path.Combine(Application.dataPath, "DataTable/Tables/ids", abFileName);
#else
            ...
            {
                // 如果读写区存在表文件, 那么使用此文件
                string fullPathFile = Path.Combine(CommonUtils.GetInternalDir(), "Tables/ids", indexFileName + ".bytes");
                if (File.Exists(fullPathFile))
                {
#if !UNITY_EDITOR && !UNITY_IOS
                    return "file://" + fullPathFile;
...
#if UNITY_EDITOR
            return Path.Combine(Application.dataPath, "DataTable/Tables/ids", indexFileName + ".bytes");
#else
        ...
        private string GetDataFileNameEx()
        {
            ...
            {
                string abFileName = JPTablePrefix + dataFileName;
                if (TableManager.Instance.DataTableAssets.ContainsKey(abFileName))
                    dataFileName = abFileName;
            }
            else if (IsUseCustomMovie ())
            {
                var fileName = AppFlyerSwitchMovieController.Logic.GetMovieTablePrefix () + dataFileName;
                if (TableManager.Instance.DataTableAssets.ContainsKey(fileName))
        ...
        private string GetIndexFileNameEx()
        {
            ...
            {
                string abFileName = JPTablePrefix + indexFileName;
                if (TableManager.Instance.DataTableAssets.ContainsKey(abFileName))
                    indexFileName = abFileName;
            }
            else if (IsUseCustomMovie())
            {
                string abFileName = AppFlyerSwitchMovieController.Logic.GetMovieTablePrefix () + indexFileName;
                if (TableManager.Instance.DataTableAssets.ContainsKey(abFileName))
                    indexFileName = abFileName;
            }
        ...
        public bool IsUseAbTest ()
        {
            return GameEntry.Controller.IsValid () &&
                   GameEntry.Controller.ABTest.IsAbTestOpen (GetTableFileName ());
        }

        public bool IsUseJapanseMovie()
        {
            if (!GetTableFileName().Equals("movie"))
                return false;

            return PlatformUtils.IsJapanAccount();
        }

        private bool IsUseCustomMovie ()
        {
            if (!GetTableFileName().Equals("movie"))
                return false;
            
            var dataFileName = this.GetDataFileName();
            
            var prefix = AppFlyerSwitchMovieController.Logic.GetMovieTablePrefix ();
            if (prefix.IsNullOrEmpty ())
                return false;

            if (!TableManager.Instance.IsValidTable (prefix + GetTableFileName ()))
            {
                return false;
            }
